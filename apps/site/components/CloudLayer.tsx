...
function useFadeIn(duration = 1.5) {
  ...
  useEffect(() => {
    ...
    function animate(ts: number) {
      ...
      setOpacity(Math.min(elapsed / duration, 1) * 0.6); // fade to 0.6
...
function MovingClouds({ fadeOpacity }: { fadeOpacity: number }) {
  ...
  // Gera posições iniciais fixas e espaçadas no eixo Z para evitar glitch
  ...
  // Use state apenas para forçar re-render, mas mantenha posições em ref
  ...
  useFrame((_, delta) => {
    ...
    positionsRef.current.forEach((cloud, i) => {
      ...
      if (cloud.z > 50) {
        ...
        // scale permanece igual para não "pular"
        changed = true;
      }
      // Atualiza a posição do mesh diretamente
      const mesh = cloudRefs.current[i];
      if (mesh) {
        mesh.position.set(cloud.x, cloud.y, cloud.z);
      }
    });
    // Força re-render apenas se houve reset de posição
