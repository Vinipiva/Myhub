...
export default function Contact() {
  ...
  function validate(form: HTMLFormElement) {
    ...
    if (!form["full-name"].value.trim()) newErrors["full-name"] = "Full name is required.";
    if (!form.email.value.trim()) newErrors.email = "Email is required.";
    else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(form.email.value.trim()))
  ...
  return (
    <section id="contact" className="isolate px-2 sm:px-6 py-12 sm:py-24 sm:py-32 lg:px-8 relative overflow-hidden">
      <div
        className="absolute bottom-0 -z-10 overflow-hidden blur-3xl overflow-hidden"
        aria-hidden="true"
      >
        <div
      ...
      <div className="mx-auto max-w-6xl mt-8 sm:mt-16 sm:mt-20 grid grid-cols-1 xl:grid-cols-2 gap-8 sm:gap-16">
        {/* Left: Other contact options */}
        <div className="flex flex-col items-start justify-start gap-8">
          <div>
            <h3 className="text-lg md:text-2l font-bold text-gray-800 mb-4">Contact info</h3>
        ...
        >
          ...
          <div className="mt-10 flex flex-col gap-4">
            ...
            {Object.values(errors).length > 0 && Object.values(errors).some(Boolean) && (
            ...
            <button
              type="submit"
              disabled={submitting}
              className="mx-auto md:mx-0 mt-0 text-xl transtion group flex h-15 w-56 items-center justify-center rounded-full bg-gradient-to-r from-purple-500 via-red-500 to-yellow-500 p-[1.5px] text-white duration-300 hover:bg-gradient-to-l hover:shadow-2xl hover:shadow-purple-600/30 cursor-pointer"
            >
              <div className="flex px-4 h-full w-full items-center justify-center rounded-full bg-gray-900 transition duration-300 ease-in-out group-hover:bg-gradient-to-br group-hover:from-gray-700 group-hover:to-gray-900 group-hover:transition group-hover:duration-300 group-hover:ease-in-out">
                {submitting ? "Sending..." : "Send your message"}
              </div>
            </button>
          </div>
          {/* Opcional: evitar captcha do formsubmit */}
          <input type="hidden" name="_captcha" value="false" />
          {/* Opcional: redirecionamento customizado */}
          {/* <input type="hidden" name="_next" value="URL_DE_SUCESSO" /> */}
        </form>
      </div>
    </section>
  );
